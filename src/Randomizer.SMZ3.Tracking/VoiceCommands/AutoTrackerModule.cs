...
namespace Randomizer.SMZ3.Tracking.VoiceCommands
{
    public class AutoTrackerModule : TrackerModule
    {
        ...
        public AutoTrackerModule(Tracker tracker, ILogger<AutoTrackerModule> logger) : base(tracker, logger)
        {
            ...
            {
                CheckZeldaLocations(message, LocationMemoryType.ZeldaRoom, true);
                _previousResponses[message.Address] = message.Bytes;
            });
            ...
            {
                CheckZeldaLocations(message, LocationMemoryType.ZeldaNPC, false);
                _previousResponses[message.Address] = message.Bytes;
            });
            ...
            {
                CheckZeldaLocations(message, LocationMemoryType.ZeldaOverworld, false);
                _previousResponses[message.Address] = message.Bytes;
            });
            ...
            AddEvent("read_block", 0x7ef300, 0xD0, Game.Zelda, (AutoTrackerMessage message) =>
            {
                ...
                CheckZeldaLocations(message, LocationMemoryType.ZeldaMisc, false);
                _previousResponses[message.Address] = message.Bytes;
            });
        ...
        protected void SendMessages(Socket socket)
        {
            ...
            while (socket.Connected)
            {
                try
                {
                    ...
                    socket.Send(Encoding.ASCII.GetBytes(JsonSerializer.Serialize(_requestMessages[currentRequestIndex]) + "\0"));
                    currentRequestIndex = (currentRequestIndex + 1) % _requestMessages.Count;
        ...
        protected void CheckZeldaLocations(AutoTrackerMessage message, LocationMemoryType type, bool is16)
        {
            foreach (var locationInfo in _tracker.WorldInfo.Locations.Where(x => x.MemoryType == type))
            {
                try
                {
                    ...
                    if (!location.Cleared && ((is16 && Check16(message.Bytes, memLocation, memFlag)) || (!is16 && Check8(message.Bytes, memLocation, memFlag))))
                    {
                        _tracker.Clear(location);
                        _tracker.TrackItem(_tracker.Items.Where(x => x.InternalItemType == location.Item.Type).First());
                    }

                    _logger.LogInformation("Location name: " + location.Name);
                }
                catch (Exception e)
                {
