...
public abstract class TrackerModule
{
    ...
    public void LoadInto(List<SpeechRecognitionGrammar> grammar)
    {
        ...
        // {
        //     engine.LoadGrammar(grammar.BuildSystemSpeechGrammar());
        // }
    }
    ...
    public static List<SpeechRecognitionGrammar> Rules = [];
        ...
        Action<SpeechRecognitionResult> executeCommand)
    {
        ...
        grammarBuilder.GrammarElement.Key = ruleName;

        try
        {
            var grammar = grammarBuilder.BuildGrammar(ruleName);
            Rules.Add(grammar);
            // Permutations.AddRange(grammarBuilder.GeneratePermutations());
    ...
    protected virtual List<GrammarKeyValueChoice> GetItemNames(Func<Item, bool>? where = null)
    {
        ...
        foreach (var item in WorldQueryService.LocalPlayersItems().Where(where))
        {
            ...
            if (item.Metadata.SpeechRecognitionNames != null)
            {
                foreach (var name in item.Metadata.SpeechRecognitionNames)
                {
                    if (!addedNames.Contains(name))
                    {
                        itemNames.Add(new GrammarKeyValueChoice(name, item.Name));
                        addedNames.Add(name);
                    }
                }
            }
    ...
    protected virtual List<GrammarKeyValueChoice> GetDungeonNames(bool includeDungeonsWithoutReward = false)
    {
        ...
        foreach (var dungeon in TrackerBase.World.TreasureRegions)
        {
            ...
            if (dungeon.Metadata.SpeechRecognitionNames != null)
            {
                foreach (var name in dungeon.Metadata.SpeechRecognitionNames)
                {
                    dungeonNames.Add(new GrammarKeyValueChoice(name, dungeon.Name));
                }
            }
        }
    ...
    protected virtual List<GrammarKeyValueChoice> GetBossNames()
    {
        ...
        foreach (var boss in TrackerBase.World.AllBosses)
        {
            ...
            if (boss.Metadata.SpeechRecognitionNames != null)
            {
                foreach (var name in boss.Metadata.SpeechRecognitionNames)
                {
                    bossNames.Add(new GrammarKeyValueChoice(name, boss.Name));
                }
            }
        }
        return bossNames;
    ...
    protected virtual List<GrammarKeyValueChoice> GetLocationNames()
    {
        ...
        foreach (var location in TrackerBase.World.Locations)
        {
            ...
            if (location.Metadata.SpeechRecognitionNames != null)
            {
                foreach (var name in location.Metadata.SpeechRecognitionNames)
                {
                    locationNames.Add(new GrammarKeyValueChoice(name, (int)location.Id));
                }
            }
        }
    ...
    protected virtual List<GrammarKeyValueChoice> GetRoomNames()
    {
        ...
        foreach (var room in TrackerBase.World.Rooms)
        {
            ...
            if (room.Metadata.SpeechRecognitionNames != null)
            {
                foreach (var name in room.Metadata.SpeechRecognitionNames)
                {
                    roomNames.Add(new GrammarKeyValueChoice(name, roomName));
                }
            }
    ...
    protected virtual List<GrammarKeyValueChoice> GetRegionNames(bool excludeDungeons = false)
    {
        ...
        foreach (var region in TrackerBase.World.Regions)
        {
            ...
            if (region.Metadata.SpeechRecognitionNames != null)
            {
                foreach (var name in region.Metadata.SpeechRecognitionNames)
                {
                    regionNames.Add(new GrammarKeyValueChoice(name, regionName));
                }
            }
        }
    ...
    protected virtual List<GrammarKeyValueChoice> GetMedallionNames()
    {
        ...
        foreach (var medallion in medallionTypes.Where(x => x == ItemType.Nothing || x.IsInCategory(ItemCategory.Medallion)))
        {
            ...
            if (item?.Metadata.SpeechRecognitionNames != null)
            {
                foreach (var name in item.Metadata.SpeechRecognitionNames)
                {
                    medallions.Add(new GrammarKeyValueChoice(name, item.Name));
                }
            }
        }
