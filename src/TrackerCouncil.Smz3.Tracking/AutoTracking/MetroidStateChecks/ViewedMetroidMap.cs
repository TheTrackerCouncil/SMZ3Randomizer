...
public class ViewedMetroidMap(ISnesConnectorService snesConnectorService, ILogger<ViewedMetroidMap> logger) : IMetroidStateCheck
{
    ...
    {
        if (tracker.World.Config.GameMode != GameMode.ParsedRom || !currentState.IsLookingAtMap || _isMakingRequest || tracker.GameStateTracker.CurrentRegion is not SMRegion region || _viewedMaps.Contains(region))
        {
            return false;
        }
