...
public class LocationsPatch : RomPatch
{
    ...
    private ushort GetMetroidItemValue(PatcherServiceData data, Location location)
    {
        var plmId = PatcherServiceData.EnableMultiworld ?
            0xEFE0 :
    ...
    private byte GetZ3ItemId(PatcherServiceData data, Location location)
    {
        var item = location.Item;
        var itemType = item.Type;
        var value = location.Type == LocationType.NotInDungeon ||
            !(item.IsDungeonItem && location.Region.IsRegionItem(item) && item.World == data.LocalWorld) ? itemType : item switch
            {
