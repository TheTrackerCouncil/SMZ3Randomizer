...
public class FairyPondTradePatch : RomPatch
{
    ...
    {
        if (!data.LocalWorld.Config.CasPatches.RandomizedBottles) yield break;
        yield return (Snes(0x6C8FF), new[] { s_fairyPondTrades.Random(data.Random) });
        yield return (Snes(0x6C93B), new[] { s_fairyPondTrades.Random(data.Random) });
    }
