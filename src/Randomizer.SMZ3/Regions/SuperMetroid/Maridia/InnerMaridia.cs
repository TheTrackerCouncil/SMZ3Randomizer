    ...
    public class InnerMaridia : SMRegion, IHasReward
    {
        public InnerMaridia(World world, Config config) : base(world, config)
        {
            PseudoSparkRoom = new(this, 142, 0x8FC533, LocationType.Visible,
                ...
                access: items => items.CardMaridiaL1 && Logic.CanPassBombPassages(items),
                memoryAddress: 0x11,
                memoryFlag: 0x40);
            ...
            RightSandPitLeft = new(this, 146, 0x8FC5EB, LocationType.Visible,
                ...
                vanillaItem: ItemType.Missile,
                access: items => CanReachAqueduct(items) && items.Super,
                memoryAddress: 0x12,
                memoryFlag: 0x4);
            ...
            ShaktoolItem = new(this, 150, 0x8FC6E5, LocationType.Chozo,
                ...
                access: items => items.Super && items.Grapple && Logic.CanUsePowerBombs(items) && (items.SpaceJump || items.HiJump),
                memoryAddress: 0x12,
                memoryFlag: 0x40);
            PreDraygonRoom = new(this, 151, 0x8FC74D, LocationType.Hidden,
                ...
                vanillaItem: ItemType.Missile,
                access: items => Logic.CanAccessMaridiaPortal(items)
        ...
        {
            return (items.CardMaridiaL1 && (Logic.CanFly(items) || items.SpeedBooster || items.Grapple))
                         || (items.CardMaridiaL2 && Logic.CanAccessMaridiaPortal(items));
        }

        private bool CanDefeatDraygon(Progression items)
        {
            return ((items.CardMaridiaL1 && items.CardMaridiaL2 && CanDefeatBotwoon(items)) ||
                    Logic.CanAccessMaridiaPortal(items)
                ) && items.CardMaridiaBoss && items.Gravity && ((items.SpeedBooster && items.HiJump) || Logic.CanFly(items));
        }

        private bool CanDefeatBotwoon(Progression items)
        {
            return items.SpeedBooster || Logic.CanAccessMaridiaPortal(items);
        }
