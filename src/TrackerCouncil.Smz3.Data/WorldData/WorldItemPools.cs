...
public class WorldItemPools
{
    ...
    public void SwapItems(List<SwapItemPoolRequest> swaps)
    {
        ...
        {
            var oldProgItem = progression.FirstOrDefault(x => x.Type == swapData.FromItemType);
            var oldNiceItem = nice.FirstOrDefault(x => x.Type == swapData.FromItemType);
            var oldJunkItem = junk.FirstOrDefault(x => x.Type == swapData.FromItemType);

            if (oldProgItem != null)
            {
                progression.Remove(oldProgItem);
            }
            else if (oldNiceItem != null)
            {
                nice.Remove(oldNiceItem);
            }
            else if (oldJunkItem != null)
            {
                junk.Remove(oldJunkItem);
            }
            else
            {
