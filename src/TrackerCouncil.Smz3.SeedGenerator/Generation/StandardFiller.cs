...
public class StandardFiller : IFiller
{
    ...
    public void Fill(List<World> worlds, Config config, CancellationToken cancellationToken)
    {
        ...
        // Set up the assumed inventory with the starting items
        ...
        foreach (var world in worlds)
        {
            ...
            else if (worldConfig.GameModeOptions.KeysanityGanonsTowerBigKeyLocation == KeysanityGanonsTowerBigKeyLocation.GanonsTower)
            {
        ...
        foreach (var (locationId, value) in configLocations)
        {
            ...
            else
            {
                ...
                else if (progressionItems.Any(x => x.Type == itemType))
                {
                    ...
                    var itemsRequired = Logic.GetMissingRequiredItems(location, new Progression(), out _).ToList();
    ...
    private void ApplyItemBias(List<Item> itemPool, IEnumerable<(ItemType type, double weight)> reorder)
    {
        ...
        var reorderList = reorder.ToList();
        var items = reorderList.ToDictionary(x => x.type, x => itemPool.FindAll(item => item.Type == x.type));
        itemPool.RemoveAll(item => reorderList.Any(x => x.type == item.Type));

        /* Insert items from each biased type such that their lowest index
         * is based on their weight on the original pool size
         */
        foreach (var (type, weight) in reorderList.OrderByDescending(x => x.weight))
        {
    ...
    private void InitialFillInOwnWorld(List<Item> dungeonItems, List<Item> progressionItems, World world, List<Item> startingInventory)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (itemsToAdd.Count > 0)
        {
            ...
            var inventory = CollectItems(itemsToAdd.Concat(initialInventory), worldsList).ToList();
            var currentRewards = CollectRewards(inventory, allRewards, worldsList).ToList();
            var currentBosses = CollectBosses(inventory, currentRewards, allBosses, worldsList);

            var location = locationsList.Empty().CanFillWithinWorld(item, inventory, currentRewards, currentBosses).FirstOrDefault();
            if (location == null)
            {
            ...
            {
                _ = locationsList.Shuffle(Random);
    ...
    {
        var worldsList = worlds.ToList();
        var progressions = worldsList.ToDictionary(w => w, w => new Progression(items.Where(i => i.World == w), new List<Reward>(), new List<Boss>()));
    ...
    {
        var worldsList = worlds.ToList();
        var progressions = worldsList.ToDictionary(w => w, w => new Progression(items.Where(i => i.World == w), rewards.Where(r => r.World == w), new List<Boss>()));
